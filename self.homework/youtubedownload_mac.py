...
if main == 1 :
    ...
    yt.streams.filter(adaptive=True, file_extension='mp4', only_video=True).order_by\
        ('resolution').desc().first().download('/Users/byeon-yunseob/Downloads/only_video', yt.title)
    #오디오 다운로드
    yt.streams.filter(adaptive=True, file_extension='mp4', only_audio=True).order_by('abr').desc().first()\
        .download('/Users/byeon-yunseob/Downloads/only_sound', yt.title)
     # 다운받는 파일 경로
    videoclip = VideoFileClip("//Users//byeon-yunseob//Downloads//only_video//" + yt.title)
    audioclip = AudioFileClip("//Users//byeon-yunseob//Downloads//only_sound//" + yt.title)
    # 오디오를 먼저 불러온다.
    videoclip.audio = audioclip
    # 그 위에 영상을 합쳐서 파일이름으로 저장한다.
    # (경로를 지정해줘도 된다. 지정하지 않으면 자동으로 파이썬 파일이 있는 곳에 저장됨)
    videoclip.write_videofile("//Users//byeon-yunseob//Downloads//" + yt.title +".mp4")
    ...
    for video in p.videos:
        yt2.streams.filter(adaptive=True, file_extension='mp4', only_audio=True).order_by('abr').desc().first() \
            .download('/Users/byeon-yunseob/Downloads//', yt2.title)
